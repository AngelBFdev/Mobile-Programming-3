...
this.item = navParams.get('dato');
  ...
  actualizarProducto(item) {
    const uri = 'https://cursodemoviles.000webhostapp.com/api.php?comando=editar&nombre=' + item.nombre
  ...
  eliminarProducto(id) {
    const uri = 'https://cursodemoviles.000webhostapp.com/api.php?comando=eliminar&id=' + id;
    this.http.get(uri).subscribe(async data => {
      this.respuesta = data;
      const mensaje = this.respuesta.mensaje;
      if (!!mensaje) {
